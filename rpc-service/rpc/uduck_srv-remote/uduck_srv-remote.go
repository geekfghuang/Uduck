...
func main() {
  ...
  client := rpc.NewUduckSrvClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  switch cmd {
  case "citySortAndLoca":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "CitySortAndLoca requires 1 args")
      flag.Usage()
    }
  ...
  case "userSex":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "UserSex requires 1 args")
      flag.Usage()
    }
    argvalue0 := flag.Arg(1)
    value0 := argvalue0
